<template>
  <div class="form" @click="closeUl">
    <form>
      <h2 class="error" v-if="russianSymbolsError">
        Поля: Фамилия, Имя, Отчество, Предыдущая Фамилия, Предыдущее Имя должны
        содержать только русские буквы!
      </h2>
      <h2 class="error" v-if="birthdayError">
        Введите валидную дату рождения!
      </h2>
      <h2 class="error" v-if="emailError">Введите валидный email!</h2>
      <h2 class="error" v-if="seriesPassError && formData.cShip === 'Russia'">
        Серия паспорта состоит из 4 цифр!
      </h2>
      <h2 class="error" v-if="numberPassError && formData.cShip === 'Russia'">
        Номер паспорта состоит из 6 цифр!
      </h2>
      <h2 class="error" v-if="latinSymbolsError && formData.cShip !== 'Russia'">
        Поля: Фамилия на латинице и Имя на латинице должны содержать только
        латинские буквы!
      </h2>
      ...
      <div class="row">
        <div class="column">
          ...
          <input
            ...
            autocomplete="off"
            @blur="validateRussianSymbols(formData.surname, $event)"
          />
        </div>
        <div class="column">
          ...
          <input
            ...
            autocomplete="off"
            @blur="validateRussianSymbols(formData.name, $event)"
          />
        </div>
        <div class="column">
          ...
          <input
            ...
            autocomplete="off"
            @blur="validateRussianSymbols(formData.patronymic, $event)"
          />
        </div>
      </div>
      <div class="column">
        ...
        <input
          ...
          @blur="validateBirthday(formData.bDate, $event)"
        />
      </div>
      <div class="column">
        ...
        <input
          ...
          autocomplete="off"
          @blur="validateEmail(formData.email, $event)"
        />
      </div>
      <h3>Пол</h3>
      <div class="row">
      ...
      <ul v-if="toggle">
        <li
          v-for="(item, idx) in filteredcShips"
          :key="idx"
          @click="onClick(item)"
        >
          {{ item }}
      ...
      <div class="row" v-if="formData.cShip === `Russia`">
        <div class="column">
          ...
          <input
            ...
            type="number"
            id="sPass"
            v-model="formData.sPass"
            @blur="validateSeriesPass(formData.sPass, $event)"
          />
        </div>
        <div class="column">
          ...
          <input
            ...
            @blur="validateNumberPass(formData.nPass, $event)"
          />
        </div>
        <div class="column">
        ...
        <div class="row">
          <div class="column">
            ...
            <input
              ...
              @blur="validateLatinSymbols(formData.latinSurname, $event)"
            />
          </div>
          <div class="column">
            ...
            <input
              ...
              @blur="validateLatinSymbols(formData.latinName, $event)"
            />
          </div>
        </div>
        ...
        <div class="row">
          <div class="column">
            ...
            <input
              ...
              type="number"
              id="altPassNumber"
              v-model="formData.altPassNumber"
            />
          </div>
          <div class="column">
          ...
          <div class="column">
            ...
            <select id="passType" v-model="formData.passType">
              <option v-for="item in passTypes" :key="item" :value="item">
                {{ item }}
              </option>
            </select>
          </div>
...
export default {
  ...
  methods: {
    ...
    getCitizenship(searchWord) {
      this.filteredcShips = this.cShips.filter((item) =>
    ...
    validateRussianSymbols(str, event) {
      this.russianSymbolsError = false;
      const REG_EXP = /[a-z A-Z0-9-().,@&#*!%:'"`~|/=+]+/;
      const validation = str.match(REG_EXP);
      if (validation) {
    ...
    validateLatinSymbols(str, event) {
    ...
    validateEmail(mail, event) {
      this.emailError = false;
      const REG_EXP = /^[A-Z0-9._%+-]+@[A-Z0-9-]+.+.[A-Z]{2,4}$/i;
      const validation = mail.match(REG_EXP);
      if (!validation) {
    ...
    validateSeriesPass(series, event) {
